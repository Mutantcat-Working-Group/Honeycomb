...
// t7-3 MQTT测试
const mqtt_broker_url = ref('mqtt://broker.emqx.io:1883');
const mqtt_client_id = ref(`client_${Math.random().toString(16).substring(2, 8)}`);
const mqtt_username = ref('');
const mqtt_password = ref('');
const mqtt_connected = ref(false);
const mqtt_client = ref(null);
const mqtt_subscribe_topic = ref('test/topic');
const mqtt_subscribe_qos = ref(0);
const mqtt_publish_topic = ref('test/topic');
const mqtt_publish_message = ref('Hello MQTT');
const mqtt_publish_qos = ref(0);
const mqtt_publish_retain = ref(false);
const mqtt_logs = ref([]);
const mqtt_auto_scroll = ref(true);
const mqtt_subscriptions = ref([]);

// 添加MQTT日志
function addMqttLog(type: string, message: string) {
    const timestamp = new Date().toLocaleTimeString();
...
function connectMqtt() {
    ...
    try {
        // 这里应该使用MQTT客户端库来连接MQTT服务器
        // 这里只是模拟连接，实际应用中需要使用MQTT客户端库
        mqtt_connected.value = true;
        addMqttLog('success', `已连接到 MQTT服务器: ${mqtt_broker_url.value}`);
    } catch (error) {
        mqtt_connected.value = false;
        addMqttLog('error', `连接失败: ${error}`);
    }
}
...
function disconnectMqtt() {
    ...
    mqtt_connected.value = false;
    addMqttLog('info', '连接已关闭');
}

// 订阅主题
function subscribeTopic() {
    if (!mqtt_connected.value) {
        addMqttLog('warning', '未连接到MQTT服务器');
        return;
    }
    
    if (!mqtt_subscribe_topic.value.trim()) {
        addMqttLog('warning', '订阅主题不能为空');
        return;
    }
    
    // 检查是否已订阅该主题
    const isAlreadySubscribed = mqtt_subscriptions.value.some(sub => sub.topic === mqtt_subscribe_topic.value);
    if (isAlreadySubscribed) {
        addMqttLog('warning', `已经订阅了主题: ${mqtt_subscribe_topic.value}`);
        return;
    }
    
    try {
        // 这里应该使用MQTT客户端库来订阅主题
        // 这里只是模拟订阅，实际应用中需要使用MQTT客户端库
        mqtt_subscriptions.value.push({
            id: Date.now(),
            topic: mqtt_subscribe_topic.value,
            qos: mqtt_subscribe_qos.value
        });
        addMqttLog('success', `已订阅主题: ${mqtt_subscribe_topic.value} (QoS: ${mqtt_subscribe_qos.value})`);
    } catch (error) {
        addMqttLog('error', `订阅主题失败: ${error}`);
    }
}
...
function publishMessage() {
    ...
    try {
        // 这里应该使用MQTT客户端库来发布消息
        // 这里只是模拟发布消息，实际应用中需要使用MQTT客户端库
        addMqttLog('info', `正在发布消息到主题: ${mqtt_publish_topic.value}`);
        addMqttLog('info', `消息内容: ${mqtt_publish_message.value}`);
    } catch (error) {
        addMqttLog('error', `发布消息失败: ${error}`);
    }
}
...
function unsubscribeTopic(topic: string) {
    ...
    try {
        // 这里应该使用MQTT客户端库来取消订阅主题
        // 这里只是模拟取消订阅，实际应用中需要使用MQTT客户端库
        mqtt_subscriptions.value = mqtt_subscriptions.value.filter(sub => sub.topic !== topic);
        addMqttLog('info', `已取消订阅主题: ${topic}`);
    } catch (error) {
        addMqttLog('error', `取消订阅失败: ${error}`);
    }
}
...
<template>
    <div class="tool-container">
        ...
        <div v-show="tooltype == 't7-3'" class="one-tool">
            <div :style="{ background: 'var(--color-fill-1)', padding: '2px' }" class="one-tool-head">
                <a-page-header :style="{ background: 'var(--color-bg-2)' }" title="MQTT测试" @back="switchToMenu"
                    subtitle="MQTT协议测试工具">
                    <template #extra>
                        <div class="can_touch">
                            <a-button class="header-button no-outline-button" @click="minimizeWindow()"> <template
            ...
            <div class="one-tool-content">
                <a-row class="page-content custom-scrollbar">
                    <a-col :span="24">
                        ...
                        <a-card title="连接设置" style="margin-bottom: 15px;">
                            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                                <div style="display: flex; align-items: center; width: 100%; margin-bottom: 10px;">
                                    <span style="width: 120px;">Broker地址：</span>
                                    <a-input v-model="mqtt_broker_url" :disabled="mqtt_connected" placeholder="MQTT Broker地址，如mqtt://broker.emqx.io:1883" 
                                            style="flex: 1; margin-right: 15px;" />
                                </div>
                                <div style="display: flex; align-items: center; width: 100%; margin-bottom: 10px;">
                                    <span style="width: 120px;">客户端ID：</span>
                                    <a-input v-model="mqtt_client_id" :disabled="mqtt_connected" placeholder="客户端唯一标识" 
                                            style="flex: 1; margin-right: 15px;" />
                                </div>
                                <div style="display: flex; align-items: center; width: 48%;">
                                    <span style="width: 120px;">用户名：</span>
                                    <a-input v-model="mqtt_username" :disabled="mqtt_connected" placeholder="可选" 
                                            style="flex: 1; margin-right: 15px;" />
                                </div>
                                <div style="display: flex; align-items: center; width: 48%;">
                                    <span style="width: 120px;">密码：</span>
                                    <a-input v-model="mqtt_password" :disabled="mqtt_connected" placeholder="可选" 
                                            style="flex: 1; margin-right: 15px;" />
                                </div>
                            </div>
                            <div style="margin-top: 15px; display: flex; justify-content: flex-end;">
                                ...
                                <a-button :disabled="!mqtt_connected" @click="disconnectMqtt">断开</a-button>
                            </div>
                            <div style="margin-top: 10px;">
                                <a-alert v-if="mqtt_connected" type="success">MQTT已连接</a-alert>
                                <a-alert v-else type="info">MQTT未连接</a-alert>
                            </div>
                        </a-card>

                        <!-- 订阅主题区 -->
                        <a-card title="订阅主题" style="margin-bottom: 15px;">
                            <div style="display: flex; align-items: center; margin-bottom: 10px;">
                                <span style="width: 80px;">主题：</span>
                                <a-input v-model="mqtt_subscribe_topic" :disabled="!mqtt_connected" placeholder="输入要订阅的主题，如test/topic" 
                                        style="flex: 1; margin-right: 15px;" />
                                <span style="width: 60px; text-align: right; margin-right: 5px;">QoS：</span>
                                <a-select v-model="mqtt_subscribe_qos" :disabled="!mqtt_connected" style="width: 80px; margin-right: 15px;">
                                    <a-option :value="0">0</a-option>
                                    <a-option :value="1">1</a-option>
                                    <a-option :value="2">2</a-option>
                                </a-select>
                                <a-button type="primary" :disabled="!mqtt_connected" @click="subscribeTopic">订阅</a-button>
                            </div>

                            <!-- 订阅列表 -->
                            <a-divider style="margin: 10px 0;">已订阅主题</a-divider>
                            <div style="max-height: 150px; overflow-y: auto; border: 1px solid #eee; border-radius: 4px; padding: 5px;">
                                <div v-if="mqtt_subscriptions.length === 0" style="text-align: center; padding: 10px; color: #888;">
                                    暂无订阅主题
                                </div>
                                <div v-else v-for="sub in mqtt_subscriptions" :key="sub.id" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; padding: 5px; border-bottom: 1px solid #eee;">
                                    <div>
                                        <a-tag color="green">{{ sub.topic }}</a-tag>
                                        <a-tag color="blue">QoS: {{ sub.qos }}</a-tag>
                                    </div>
                                    <a-button size="mini" status="danger" @click="unsubscribeTopic(sub.topic)">
                                        取消订阅
                                    </a-button>
                                </div>
                            </div>
                        ...
                        <!-- 发布消息区 -->
                        <a-card title="发布消息" style="margin-bottom: 15px;">
                            <div style="display: flex; align-items: center; margin-bottom: 10px;">
                                <span style="width: 80px;">主题：</span>
                                <a-input v-model="mqtt_publish_topic" :disabled="!mqtt_connected" placeholder="输入要发布的主题，如test/topic" 
                                        style="flex: 1; margin-right: 15px;" />
                            </div>
                            <div style="display: flex; align-items: flex-start; margin-bottom: 10px;">
                                <span style="width: 80px;">消息：</span>
                                <a-textarea v-model="mqtt_publish_message" :disabled="!mqtt_connected" placeholder="输入要发布的消息内容" 
                                          :auto-size="{ minRows: 3, maxRows: 5 }" style="flex: 1; margin-right: 15px;" />
                            </div>
                            <div style="display: flex; align-items: center; margin-bottom: 10px;">
                                <span style="width: 80px;">QoS：</span>
                                <a-select v-model="mqtt_publish_qos" :disabled="!mqtt_connected" style="width: 80px; margin-right: 15px;">
                                    <a-option :value="0">0</a-option>
                                    <a-option :value="1">1</a-option>
                                    <a-option :value="2">2</a-option>
                                </a-select>
                                <a-checkbox v-model="mqtt_publish_retain" :disabled="!mqtt_connected">
                                    保留消息
                                </a-checkbox>
                            </div>
                            <div style="margin-top: 10px; display: flex; justify-content: flex-end;">
                                <a-button type="primary" :disabled="!mqtt_connected" @click="publishMessage">发布消息</a-button>
                            </div>
                        </a-card>
                        ...
                        <a-card title="通信日志">
                            <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                                ...
                                <div>
                                    <a-button size="small" @click="clearMqttLogs">清空日志</a-button>
                                </div>
                            </div>

                            <div class="mqtt-log-container" style="height: 250px; overflow-y: auto; border: 1px solid #eee; padding: 10px; border-radius: 4px; width: 96%; margin: 0 auto;">
                                <div v-for="log in mqtt_logs" :key="log.id" class="mqtt-log-item" :class="`mqtt-log-${log.type}`" style="margin-bottom: 5px;">
                                    <span class="mqtt-log-time" style="color: #888;">[{{ log.timestamp }}]</span>
                                    ...
                                    <span class="mqtt-log-message" style="margin-left: 5px;">{{ log.message }}</span>
                                </div>
                                <div v-if="mqtt_logs.length === 0" class="mqtt-log-empty" style="text-align: center; padding: 10px; color: #888;">
                                    暂无日志记录
                                </div>
                            </div>
                        </a-card>
